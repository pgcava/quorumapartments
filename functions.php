 ...
 */
function quorum_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on Quorum, use a find and replace
		* to change 'quorum' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'quorum', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'quorum' ),
		)
	);
...
add_action( 'after_setup_theme', 'quorum_setup' );
 ...
 */
function quorum_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'quorum_content_width', 640 );
}
add_action( 'after_setup_theme', 'quorum_content_width', 0 );
 ...
 */
function quorum_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'quorum' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'quorum' ),
...
add_action( 'widgets_init', 'quorum_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function quorum_scripts() {
	wp_enqueue_style( 'quorum-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'quorum-style', 'rtl', 'replace' );

	// Abril font
	wp_enqueue_style( 'quorum-abril-display', 'https://use.typekit.net/png5xxm.css', array(), _S_VERSION );
...
add_action( 'wp_enqueue_scripts', 'quorum_scripts' );
...
function contactForm(){
	...
	$subject = '[Quorum Apartments] Wiadomość ze strony quorumapartments.pl';
	$user_message = "Imię i nazwisko: " . $name . " " . $surname . "\r\nTelefon: " . $phone . "\r\nMail: " . $mail . "\r\nWiadomość: " . $message;
